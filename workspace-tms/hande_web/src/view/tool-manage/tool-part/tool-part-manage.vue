<template>
  <div>
        ...
        <Form ref="formInline" :model="searchCondition" :label-width="65" inline>
          ...
          <Col span="8">
            <FormItem style="width: 100%;text-align: right;">
              ...
              <Button type="success" style="margin-right: 5px;" @click="addToolPart">
    ...
    <Modal
      v-model="toolPartDetailModalshow"
      :title="isModify?'修改物料制件':'新增物料制件'"
      width="600"
      draggable
    >
...
import tool_part_form_config from "./tool-part-form-config.js";
...
export default {
  data() {
    return {
      ...
      isBatchDetele: false,
      uploadAction:
        getBaseUrl() + "/base/toolpart-import.json?token=" + getToken(),
      searchCondition: {
        typeId: null,
        toolNumber: "",
        partCode: "",
      ...
      formValidate: tool_part_form_config.formValidate, //user form表单字段
      ruleValidate: tool_part_form_config.ruleValidate //user form表单验证规则
    };
  },
  created() {
    const self = this;
  ...
  methods: {
    getListData() {
      ...
      axios
        .request({
          ...
          url: "/base/toolpart-page-list.json",
          method: "post",
          isAuth: true,
    ...
    addNewToolPart() {
      ...
      const self = this;
      self.isModify = false;
      self.toolPartDetailModalshow = true;
      let para = {
        toolId: self.toolId,
      ...
      axios
        ...
        .then(function(res) {
          const data = res.data;
          if (data.code === 200) {
            self.toolPartDetailModalshow = false;
            self.$Message.success("物料制件新增成功!");
    ...
    searchToolBase() {
      const self = this;
      if (!!!this.formValidate.toolNumber) {
        return;
      }
      let para = {
    ...
    deleteRowDate(pkId, toolNumber, partCode) {
      let para = {
        pkId: pkId,
      ...
      axios
        ...
        .then(function(res) {
          ...
          if (res.data.code === 200) {
            self.$Message.success("物料制件信息删除成功!");
            self.getListData();
          }
        })
        .catch(function(err) {
    ...
    handleReset(name) {
      ...
      this.toolPartDetailModalshow = false;
    },
    exportData() {
      const self = this;
      window.location.href =
        getBaseUrl() + "/base/tool-part-export?token=" + getToken();
    },
    handleSuccess() {
      const self = this;
      self.getListData();
