<template>
  <div>
        ...
        <Form ref="formInline" :model="searchCondition" :label-width="75" inline>
          ...
          <Col span="8">
            <FormItem style="width:100%;text-align:right">
              ...
              <Button type="success" style="margin-right: 5px;" @click="addRepairPrice">
    ...
    <Modal
      v-model="repairPriceDetailModalshow"
      :title="isModify?'修改刃磨价格':'新增刃磨价格'"
      width="600"
      draggable
    >
...
import repair_price_form_config from "./repair-price-form-config.js";
...
export default {
  data() {
    return {
      ...
      toolName: "",
      supplierName: "",
      repairPriceDetailModalshow: false,
      deleteModalShow: false, //控制删除modal提示 显示
      deleteRowData: {}, //行删除行数据
      isModify: false, //是否是修改明细操作
      uploadAction:
        getBaseUrl() + "/base/repair-supplier-import.json?token=" + getToken(),
      supplierList: [],
      isBatchDetele: false,
      searchCondition: {
        toolNumber: "",
      ...
      formValidate: repair_price_form_config.formValidate, //user form表单字段
      ruleValidate: repair_price_form_config.ruleValidate //user form表单验证规则
    };
  },
  created() {
    const self = this;
  ...
  methods: {
    getListData() {
      ...
      axios
        .request({
          ...
          url: "/base/repair-price-page-list.json",
          method: "post",
          isAuth: true,
    ...
    addNewRepairPrice() {
      ...
      const self = this;
      self.repairPriceDetailModalshow = true;
      let para = {
        toolId: self.toolId,
      ...
      axios
        .request({
          ...
          url: "/base/repair-price-add.json",
          method: "post",
          isAuth: true,
          params: para
        })
        .then(function(res) {
          const data = res.data;
          if (data.code === 200) {
            self.repairPriceDetailModalshow = false;
            self.$Message.success("刃磨价格新增成功!");
    ...
    editRepairPrice() {
      ...
      axios
        .request({
          ...
          url: "/base/repair-price-update.json",
          method: "post",
          isAuth: true,
          params: para
        })
    ...
    deleteRowDate(pkId, supplierName, toolNumber) {
      ...
      axios
        .request({
          ...
          url: "/base/repair-price-delete-by-id",
          method: "post",
          isAuth: true,
          params: para
        })
    ...
    exportData() {
      ...
      window.location.href =
        getBaseUrl() + "/base/repair-price-export?token=" + getToken();
    },
    //form 校验方法
    handleSubmit(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          if (this.isModify) {
            this.editRepairPrice();
          } else {
            this.addNewRepairPrice();
          }
        } else {
    ...
    handleReset(name) {
      this.repairPriceDetailModalshow = false;
