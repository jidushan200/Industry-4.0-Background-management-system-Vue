      ...
      <Row>
        <Col span="6">
          <FormItem label="物料条码" prop="fullNumber">
            <Input style="width:150px;" v-model="formValidate.fullNumber"></Input>
          </FormItem>
        </Col>
        <Col span="6">
          ...
          <FormItem label="附件" prop="appendixIds">
            ...
            >
              <template v-if="item.status === 'finished'">
                <a type="dashed" :key="item.id" @click="downFile(item.id)">
                  {{
                  item.sourceName
                  }}
                </a>
    ...
    searchCheckStandard () {
      ...
      axios
        .request({
          ...
          url: '/base/check-standard-get-by-full-number.json',
          method: 'post',
          isAuth: true,
          params: para
        })
        .then(function (res) {
          ...
          if (data.code === 200) {
            if (data.data) {
              ...
              self.formValidate.toolName = data.data.tool.toolName
              self.formValidate.toolNumber = data.data.tool.toolNumber
              self.toolName = self.formValidate.toolName
              self.toolMap = data.data.tool.toolMap
              self.typeId = data.data.tool.typeId
              self.warehouseCode = data.data.tool.warehouseCode
