<template>
  <Modal v-model="measurePurchaseReceipt" title="采购收货" width="1100" draggable>
...
import receipt_form_config from "./measure-purchase-receipt-form-config.js";
export default {
  name: "measurePurchaseReceipt",
  data() {
    return {
      isDisabled: false,
      measurePurchaseReceipt: false,
      applierName: "",
      availableNumber: "",
      seqArray: [],
      data: [],
  ...
  methods: {
    show(row) {
      const self = this;
      self.data = [];
      self.measurePurchaseReceipt = true;
    ...
    saveReceipt() {
      ...
      axios
        ...
        .then(function(res) {
          self.measurePurchaseReceipt = false;
          if (res.data.code === 200) {
    ...
    handleReset(name) {
      this.measurePurchaseReceipt = false;
