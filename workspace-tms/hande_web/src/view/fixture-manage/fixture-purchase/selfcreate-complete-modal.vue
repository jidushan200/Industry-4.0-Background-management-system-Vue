...
<template>
  <!--申请单信息弹出modal-->
  <Modal v-model="selfcreateCompleteModalshow" class="modal-class" title="完工汇报" width="1160" draggable>
    <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="95" inline>
      ...
      <Row>
        ...
        <FormItem label="申购类型" prop="purchaseType">
          <Select style="width:180px" v-model="purchaseType" ref="purchaseType" placeholder="请选择采购类型">
            <Option v-for="item in purchaseTypeList" :value="item.value" :key="item.value">{{ item.name }}</Option>
          </Select>
        </FormItem>
        </Col>
        <Col span="6">
      ...
      <Row>
        ...
        <FormItem label="申购原因" prop="purchaseResion">
          <Select style="width:180px" v-model="purchaseResion" ref="purchaseResion" placeholder="请选择采购原因">
            <Option v-for="item in purchaseResionList" :value="item.value" :key="item.value">{{ item.name }}</Option>
          </Select>
        </FormItem>
        </Col>
        <Col span="6">
        <FormItem label="需求时间" prop="demandTime">
          <DatePicker type="date" style="width: 180px" :options="dateOption" v-model="demandTime" readonly></DatePicker>
        </FormItem>
        </Col>
        <Col span="6">
      ...
      <Row>
        ...
        <FormItem label="可用顺序号" prop="availableNumber">
          <Input v-model="availableNumber" style="width:1020px;" readonly />
        </FormItem>
        </Col>
      </Row>
      <Table
      ...
      <Table border :columns="columns" :data="data" height="240"></Table>
    </Form>
    <div slot="footer">
...
export default {
  data() {
    return {
      ...
      demandTime: null,
      applyTime: null,
      selfcreateCompleteModalshow: false,
  ...
  methods: {
    showModal(pkId) {
      ...
      self.selfcreateCompleteModalshow = true;
    },
    handleReset() {
      const self = this;
      self.selfcreateCompleteModalshow = false;
    },
      ...
      axios
        ...
        .then(function(res) {
          ...
          if (data.code === 200 && data.data !== null) {
            ...
            //self.applyStatus = detail.applyStatus;
    ...
    getCurrentStaff() {
      ...
      axios
        ...
        .then(function(res) {
          const data = res.data;
          if (data.code === 200 && data.data !== null) {
            self.deliverer = data.data.staffName;
          }
        });
    },
