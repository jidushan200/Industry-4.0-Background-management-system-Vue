<template>
  <div>
    ...
    <selfcreate-complete-modal ref="selfcreateCompleteModal" @search="getListData"></selfcreate-complete-modal>
  </div>
</template>
<script>
...
import selfcreateCompleteModal from "./selfcreate-complete-modal.vue";
export default {
  name: "page",
  computed: {
    access() {
    ...
    //保存、提交、删除（生产部）
    purchaseAuth() {
      return hasOneOf(["010504"], this.access);
    }
  },
  components: {
    purchaseApplyModal,
    selfcreateCompleteModal
  },
  data() {
    return {
      pkId: null,
            ...
            return h("div", [
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.showDeliveryDetail(params.row.pkId);
                    }
                  }
                },
                "完工"
  ...
  methods: {
    ...
    showDetail(pkId, applyStatus) {
      const self = this;
      self.$refs.purchaseApplyModal.showModal(pkId, applyStatus);
    },
    //收货详情
    showDeliveryDetail(pkId) {
      const self = this;
      self.$refs.selfcreateCompleteModal.showModal(pkId);
    }
  }
};
</script>