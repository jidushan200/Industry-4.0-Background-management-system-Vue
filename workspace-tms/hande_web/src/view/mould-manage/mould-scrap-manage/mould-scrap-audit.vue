<template>
  <div>
    ...
    <Modal v-model="mouldApplyModalshow" :title="isModify?'报废审批单':'报废申请单'" width="900" draggable>
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="85" inline>
        <Row>
          <Col span="8">
            <FormItem label="模具条码" prop="fullNumber">
              <Input
                style="width:180px"
          ...
          <Col span="8">
            <FormItem label="模具图号" prop="mouldMap">
              <Input style="width:180px" v-model="mouldMap" readonly />
            </FormItem>
          </Col>
          <Col span="8">
            <FormItem label="模具名称" prop="mouldName">
              <Input style="width:180px" v-model="mouldName" readonly></Input>
            </FormItem>
          </Col>
        </Row>
        <Row>
          <Col span="8">
            <FormItem label="已加工数量" prop="processAmount">
              <Input style="width:180px" v-model="processAmount" readonly></Input>
            </FormItem>
          </Col>
          <Col span="8">
          ...
          <Col span="8">
            <FormItem label="申请部门" prop="applyDepartment">
              <Input style="width:180px" v-model="applyDepartment" readonly />
            </FormItem>
          </Col>
        </Row>
        <Row>
          <Col span="8">
            <FormItem label="申请人" prop="applierCode">
              <Input style="width:180px" v-model="staffInfo" readonly></Input>
            </FormItem>
          </Col>
          <Col span="16">
...
import mould_scrap_manage_form_config from "./mould-scrap-manage-form-config.js";
...
export default {
  ...
  computed: {
    ...
    //保存、提交、删除（质检部）
    applyAuth() {
      return hasOneOf(["0106010401"], this.access);
    },
    //审核(锻造研究所)
    technologyAuth() {
      return hasOneOf(["0106010402"], this.access);
    }
  },
  components: {
    auditLogModel
  ...
  data() {
    return {
      ...
      mouldApplyModalshow: false, //控制明细modal显示
      ...
      formValidate: mould_scrap_manage_form_config.formValidate, //user form表单字段
      ruleValidate: mould_scrap_manage_form_config.ruleValidate //user form表单验证规则
    };
  },
  created() {
    const self = this;
  ...
  methods: {
    getListData() {
      ...
      axios
        .request({
          ...
          url: "/mould/mould-scrip-apply-page-list.json",
          method: "post",
          isAuth: true,
          params: para
        })
    ...
    addNewApply() {
      ...
      axios
        .request({
          ...
          url: "/mould/mould-scrip-apply-add",
          method: "post",
          isAuth: true,
          params: para
        })
        .then(function(res) {
          self.mouldApplyModalshow = false;
    ...
    editApply() {
      ...
      axios
        .request({
          ...
          url: "/mould/mould-scrip-apply-update",
          method: "post",
          isAuth: true,
          params: para
        })
    ...
    deleteRowDate(pkId, fullNumber) {
      ...
      axios
        .request({
          ...
          url: "/mould/mould-scrip-apply-delete-by-id",
          method: "post",
          isAuth: true,
          params: para
        })
    ...
    show(index) {
      const self = this;
      self.mouldApplyModalshow = true;
      self.isModify = true;
      self.formValidate = objCopy(self.tableData.rows[index], {});
      self.mouldNumber = self.formValidate.mouldNumber;
      self.mouldMap = self.formValidate.mouldMap;
      self.mouldName = self.formValidate.mouldName;
      ...
      if (auditStatus == 0 || auditStatus == -1) {
        ...
        self.applyDepartment = self.formValidate.departmentName;
        self.staffInfo = self.formValidate.applierName;
      ...
      if (!!!self.formValidate.fullNumber) {
        ...
        axios
          .request({
            ...
            url: "/mould/mould-get-by-full-number.json",
            method: "post",
            isAuth: true,
            params: para
          })
          .then(function(res) {
            const data = res.data;
            if (data.code === 200) {
              if (data.data.mouldStatus == 7) {
                self.$Message.error("模具已经报废");
                self.formValidate.fullNumber = "";
                return;
              }
              self.mouldMap = data.data.mouldMap;
              self.mouldNumber = data.data.mouldNumber;
              self.mouldName = data.data.mouldName;
              self.processAmount = data.data.processAmount;
              self.formValidate.mouldName = data.data.mouldName;
              self.formValidate.mouldMap = data.data.mouldMap;
            } else {
              self.$Notice.error({
                title: "错误提示",
                desc: res.data.info
    ...
    handleAudit() {
      ...
      axios
        .request({
          ...
          url: "/mould/scrip-report-audit.json",
          method: "post",
          isAuth: true,
          params: para
        })
    ...
    handleReset(name) {
      this.mouldApplyModalshow = false;
